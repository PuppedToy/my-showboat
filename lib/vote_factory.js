...
VoteFactory.prototype.newVote = function(event, host, socket) {

	let existing_vote = this.searchVoteByHostId(host._id);
	let new_vote = null;

	if(existing_vote) {
		existing_vote.setSocket(socket);
		existing_vote.setHost(host);
		new_vote = existing_vote;
	} else {
		let new_id = this.generateId();
		if(new_id) {
			new_vote = new Vote(new_id, event, host, socket);
